...
const V202_GENERIC_FALLBACK_EXTRA = {
  ...
  strimmer:        ['Bosch UniversalGrassCut 18V-260 Cordless', 'Makita DUR181Z 18V LXT', 'Stihl FSA 57 Battery Strimmer'],
  grasstrimmer:    ['Bosch UniversalGrassCut 18V-260 Cordless', 'Makita DUR181Z 18V LXT', 'Stihl FSA 57 Battery Strimmer'],
  lawnmower:       ['Flymo EasiStore 300R Li Cordless', 'Bosch Rotak 36 R Corded', 'Honda HRX 426 PD Petrol'],
  mower:           ['Flymo EasiStore 300R Li Cordless', 'Bosch Rotak 36 R Corded', 'Honda HRX 426 PD Petrol'],
  hedgetrimmer:    ['Bosch AHS 50-20 LI Cordless', 'Makita DUH523Z 18V LXT', 'Stihl HSA 56 Cordless'],
  leafblower:      ['Bosch ALB 18 LI Cordless', 'Ryobi RBV3000CESV Electric', 'Stihl BGA 57 Battery Blower'],
  pressurewasher:  ['Karcher K2 Compact', 'Bosch UniversalAquatak 130', 'Karcher K4 Power Control'],
  drill:           ['Bosch PSB 1800 LI-2', 'DeWalt DCD777C2 Brushless', 'Makita DHP484Z 18V LXT'],
  cordlessdrill:   ['Bosch PSB 1800 LI-2', 'DeWalt DCD777C2 Brushless', 'Makita DHP484Z 18V LXT'],
  fryingpan:       ['Tefal Comfort Max 24cm Non-Stick', 'ProCook Gourmet Stainless Steel 28cm', 'Le Creuset Toughened Non-Stick 28cm'],
};
...
async function _v202InnerHandler(req, res) {
      ...
      if (_v201GenericTiers && _v201GenericTiers.length === 3) {
        ...
        const _v201Response = {
          ...
          tiers: _v201SyntheticParsed.alternatives_array.map((name, i) => ({
            tier: ['basic','top_rated','premium'][i],
            pill_text: ['BUDGET','TOP RATED','PREMIUM'][i],
            name,
            retailer: null, price_gbp: null, price_str: null,
            url: 'https://www.amazon.co.uk/s?k=' + encodeURIComponent(name),
            blurb: ['Cheapest reliable option in the category.','Most popular UK reviewer pick — best balance of price and quality.','Top-end build for heavy-duty use.'][i],
            blurb_basis: i === 1 ? 'review_count' : (i === 0 ? 'price_anchor' : 'premium_signal'),
            review_count: null, review_stars: null, image: null,
            is_placeholder: false, is_family_variant: false, is_curator: false, tag: null,
            intent_label: ['Best Value','Top Rated','Premium Choice'][i],
            rationale: ['Cheapest reliable option.','Most popular UK pick.','Top-end build quality.'][i],
            tier_label: ['Budget','Mid-tier','Premium'][i],
            est_price_range: null, typical_price_gbp: null,
          })),
          disambig_kind: 'generic',
  ...
  const _canonicalKey = `savvey:canonical:v131:${String(parsed.canonical_search_string || '').toLowerCase().replace(/[^a-z0-9]+/g, '_').slice(0, 80)}`;
